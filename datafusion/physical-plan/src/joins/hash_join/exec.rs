...
impl ExecutionPlan for HashJoinExec {
    ...
    fn gather_filters_for_pushdown(
        ...
        // then checks column references to route each predicate to the correct side. We apply
        // the same two-level logic here: `lr_is_preserved` gates whether a side is eligible at
        // all, and `ChildFilterDescription::from_child` checks whether each filter's columns
        // exist in that child's schema (marking it unsupported if they don't).
